(function ($) {
    ...
    $().ready(function () {
    	$.exportacao = {
    		contexto: "/portalweb/jqplot/exportToImage"
    	}
    	
    	 $.extend({  
    		 downloadChart: function(grafico){
    			 
    			 var base64 = jqplotToImg($('#'+grafico));
    			 
    			 $("#base64").val(base64);
    			 $('#form').attr('action',$.exportacao.contexto);
    			 $("#form").submit(function(e){return false;});
    			 $.fileDownload($('#form').prop('action'), {httpMethod: "POST",data: $('#form').serialize()});
    		 }
    	 });
